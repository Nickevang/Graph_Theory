...
# Perform BFS crawling with exploration of related articles for each node
def bfs_with_exploration(seed_article_title, max_nodes, max_related_articles_per_article, similarity_threshold):
    # Create a directed graph
    graph = nx.DiGraph()

    # Initialize queue for BFS crawling
    queue = [(seed_article_title, 1)]  # (article_title, depth)
    ...
    while queue:
        current_article, depth = queue.pop(0)
...
max_nodes = 1000
max_related_articles_per_article = 100
similarity_threshold = 0.5

# Perform BFS crawling with exploration of related articles for each node
graph = bfs_with_exploration(seed_article_title, max_nodes, max_related_articles_per_article, similarity_threshold)

# Save the graph to a Gephi file
nx.write_gexf(graph, "Directed_Graph_With_Weights_And_Exploration_BFS.gexf")
